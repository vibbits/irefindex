...
resource "openstack_compute_instance_v2" "irefindex" {
  ...
  key_pair    = openstack_compute_keypair_v2.vsc-key.name
...
resource "openstack_networking_portforwarding_v2" "ssh_pf" {
  ...
  provisioner "local-exec" {
    // StrictHostKeyChecking is set to no as when running this multiple times (e.g. after a terraform destroy), the host key will change and this will fail.
