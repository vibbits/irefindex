...
create temporary table tmp_participants as
    select source, filename, entry, participantid, property, array_accum(distinct refvaluepsi || '(' || coalesce(name, '-') || ')') as refvalues
    from xml_xref_participants
    left outer join psicv_terms
        on refvaluepsi = codepsi
...
create temporary table tmp_methods as
    select source, filename, entry, experimentid, property, array_accum(distinct refvaluepsi || '(' || coalesce(name, '-') || ')') as refvalues
...
create temporary table tmp_interaction_experiments as
    select I.*,
        ...
        case when typeI.refvaluepsi is null then '-'
             when typenameI.name is null then 'psi-mi:"MI:0000"(' || typeI.refvaluepsi || ')'
             else typeI.refvaluepsi || '(' || typenameI.name || ')'
    ...
    left outer join psicv_terms as typenameI
        on typeI.refvaluepsi = typenameI.codepsi
