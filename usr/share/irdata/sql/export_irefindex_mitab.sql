...
create temporary table tmp1_interactions as
...
create temporary table tmp1_aliases as
...
analyze tmp1_aliases;

-- Accumulate role collections.
-- Each role is encoded as "MI:NNNN(...)".

create temporary table tmp1_participants as
...
alter table tmp1_participants add primary key(source, filename, entry, participantid, property);
analyze tmp1_participants;

-- Accumulate methods.
-- Each role is encoded as "MI:NNNN(...)".

create temporary table tmp1_methods as
...
alter table tmp1_methods add primary key(source, filename, entry, experimentid, property);
analyze tmp1_methods;

-- Accumulate PubMed identifiers.

create temporary table tmp1_pubmed as
...
alter table tmp1_pubmed add primary key(source, filename, entry, experimentid);
analyze tmp1_pubmed;

-- Consolidate assignment information to get full details of preferred assignments.

create temporary table tmp1_assignments as
    select A.*, score
...
alter table tmp1_assignments add primary key(source, filename, entry, interactorid);
analyze tmp1_assignments;

-- Accumulate confidence score information.

create temporary table tmp1_confidence as
    select I.rigid, array_accum(distinct confI.scoretype || ':' || cast(confI.score as varchar)) as confidence
    from tmp1_interactions as I
    left outer join irefindex_confidence as confI
        on I.rigid = confI.rigid
    group by I.rigid;

alter table tmp1_confidence add primary key(rigid);
analyze tmp1_confidence;

-- Collect all interaction-related information.

create temporary table tmp1_named_interactions as
    select I.*,
        ...
        coalesce(sourceI.codepsi, 'psi-mi:"MI:0000"') || '(' || coalesce(sourceI.name, lower(I.source)) || ')' as sourcedb,
    ...
    inner join tmp1_confidence as confI
        on I.rigid = confI.rigid;

analyze tmp1_named_interactions;

-- Combine interaction and experiment information.

create temporary table tmp1_interaction_experiments as
    select I.*,
        ...
        case when typeI.refvalue is null then '-'
             when typenameI.name is null then 'psi-mi:"MI:0000"(' || typeI.refvalue || ')'
    ...
    from tmp1_named_interactions as I
    ...
    left outer join tmp1_methods as methodE
    ...
    left outer join psicv_terms as typenameI
        on typeI.refvalue = typenameI.codepsi
        and typenameI.nametype = 'preferred'

    -- PubMed identifiers.

    left outer join tmp1_pubmed as pubmedE
...
analyze tmp1_interaction_experiments;

-- Combine interactor information.
\copy tmp1_interaction_experiments to '<directory>/interaction_exp1'
create temporary table tmp1_interactor_experiments as
    select I.*,
    ...
    from tmp1_interaction_experiments as I

    -- Information for interactor A.

    left outer join tmp1_assignments as nameA
    ...
    inner join tmp1_assignments as nameB
...
analyze tmp1_interactor_experiments;
\copy tmp1_interactor_experiments to '<directory>/interactor_exp2'
-- Collect all participant-, interactor- and interaction-related information.

create temporary table tmp1_mitab_interactions as
    select I.*,

        -- biologicalRoleA

        case when edgetype = 'C' or bioroleA.refvalues is null or array_length(bioroleA.refvalues, 1) = 0 then 'psi-mi:"MI:0000"(unspecified)'
             else array_to_string(bioroleA.refvalues, '|')
        end as biologicalRoleA,

        -- biologicalRoleB

        case when bioroleB.refvalues is null or array_length(bioroleB.refvalues, 1) = 0 then 'psi-mi:"MI:0000"(unspecified)'
             else array_to_string(bioroleB.refvalues, '|')
        end as biologicalRoleB,

        -- experimentalRoleA

        case when edgetype = 'C' or exproleA.refvalues is null or array_length(exproleA.refvalues, 1) = 0 then 'psi-mi:"MI:0000"(unspecified)'
             else array_to_string(exproleA.refvalues, '|')
        end as experimentalRoleA,

        -- experimentalRoleB

        case when exproleB.refvalues is null or array_length(exproleB.refvalues, 1) = 0 then 'psi-mi:"MI:0000"(unspecified)'
             else array_to_string(exproleB.refvalues, '|')
        end as experimentalRoleB

    from tmp1_interactor_experiments as I

    -- Information for participant A.

    left outer join tmp1_participants as bioroleA
        ...
        and I.edgetype <> 'C'
    left outer join tmp1_participants as exproleA
    ...
    left outer join tmp1_participants as bioroleB
    ...
    left outer join tmp1_participants as exproleB
...
analyze tmp1_mitab_interactions;
\copy tmp1_mitab_interactions to '<directory>/int_mitab'
-- Combine with ROG-related information to produce MITAB-appropriate records.

create temporary table tmp1_mitab_all as
    select
        ...
        case when edgetype = 'C' then 'psi-mi:"MI:0315"(protein complex)' else 'psi-mi:"MI:0326"(protein)' end as interactorTypeA,

        -- interactorTypeB

        cast('psi-mi:"MI:0326"(protein)' as varchar) as interactorTypeB,
...
UPDATE tmp1_mitab_all SET updateDate=REGEXP_REPLACE(updateDate,'-','/','g');
UPDATE tmp1_mitab_all SET creationDate=REGEXP_REPLACE(creationDate,'-','/','g');
UPDATE tmp1_mitab_all SET interactiontype='psi-mi:"MI:0933"(diverging genetic interaction)'  WHERE interactiontype= 'psi-mi:"MI:0000"(psi-mi:"Mi:0933")';
UPDATE tmp1_mitab_all SET aliasa= 'hgnc:"Su(var)3-7"|uniprotkb:SUV37_DROME|crogid:RoAuo6NH8jq8dliHvYA1PY/HYtI7227|icrogid:3904603' where aliasa = 'hgnc:Su(var)3-7|uniprotkb:SUV37_DROME|crogid:RoAuo6NH8jq8dliHvYA1PY/HYtI7227|icrogid:3904603'; 
UPDATE tmp1_mitab_all SET aliasb= 'hgnc:"Su(var)3-7"|uniprotkb:SUV37_DROME|crogid:RoAuo6NH8jq8dliHvYA1PY/HYtI7227|icrogid:3904603' where aliasb= 'hgnc:Su(var)3-7|uniprotkb:SUV37_DROME|crogid:RoAuo6NH8jq8dliHvYA1PY/HYtI7227|icrogid:3904603';

analyze tmp1_mitab_all;
...
\copy tmp1_mitab_all to '<directory>/mitab_all'

-- Make special organism-specific files.

create temporary table tmp1_organisms (
    taxid integer not null
);

\copy tmp1_organisms from '<directory>/organisms.txt'
analyze tmp1_organisms;

create temporary table tmp1_interaction_taxids as
    select distinct 'rigid:' || rigid as checksumInteraction, cast(substring(rogid from 28) as integer) as taxid
    from irefindex_interactions;

analyze tmp1_interaction_taxids;
...
create temporary table tmp1_mitab_all_organisms as
    select coalesce(cast(O.taxid as varchar), 'other') as taxid, M.*
    from tmp1_mitab_all as M
    inner join tmp1_interaction_taxids as T
        on T.checksumInteraction = M.checksumInteraction
    left outer join tmp1_organisms as O
        on O.taxid = T.taxid
    order by T.taxid;

\copy tmp1_mitab_all_organisms to '<directory>/mitab_all_organisms'

create temporary table tmp1_covid as
    select *
    from tmp1_mitab_all as M
    where  taxa like '%2697049%' or taxb like '%2697049%';

\copy tmp1_covid to '<directory>/mitab_covid'
