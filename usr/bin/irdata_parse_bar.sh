...
for THISFILE in $FILENAMES; do
    ...
    awk 'BEGIN {FS="\t";}{if ($12 != "psi-mi:\"MI:1110\"(predicted interaction)"  && $12 != "-") print $0;}' < "$THISFILE" > tmp
...
"${USE_PYTHON_INTERPRETER}" "$TOOLS/irdata_parse_mitab.py" 'BAR' "$DATADIR" $FILENAMES
